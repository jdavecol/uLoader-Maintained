...
"No More Snow, Please!!!", "Automatic DVD/SD Mode", "Short Direct Launch (from Channel)", "Back in Black", "Force IOS 224 instead IOS 222", "Auto-Fallback on HUB"
	},
    // spanish
...
//22	
	...
	".dol Alternativo","Actualizaci�n de uLoader","uLoader Hacks","","",
...
"No Mas Nieve, Por Favor!!!", "Modo DVD/SD Autom�tico", "Lanzamiento Directo Corto (Desde Canal)", "Back in Black", "Fuerza IOS 224 en lugar de IOS 222", "Auto-Fallback si HUB",
	},
	};
	...
	if(exit_by_reset) {test_mode=1; SYS_SetResetCallback(reset_call);exit_by_reset=0;}
		...
		while(1)
			...
			if(ret2==-20001 || ret2==-20002) 
				...
				// escape con boton 2 para no quedarse en bucle infinito
			...
			if(ret2==-20000) 
				...
				PY+=80;
				s_printf("USB readers with built-in SD/card slot have an internal hub.");

				DrawRoundFillBox((SCR_WIDTH-540)/2, SCR_HEIGHT/2-16+32+160, 540, 64, 999, 0xa00000ff);
				DrawRoundBox((SCR_WIDTH-540)/2, SCR_HEIGHT/2-16+32+160, 540, 64, 999, 4, 0xa0000000);
				PY+=80;
				s_printf("Not supported by uLoader. Press < 2 > for DVD Mode");

				// escape automatico (mismo comportamiento que -100): FAT o lectora interna
				// (inmediato si el hack "Auto-Fallback on HUB" esta activado, bit2 de uhack_settings0)
				if((automatic_dvd_mode && (time(NULL)-rel_time)>5) || (uhack_settings0 & 4))
					{
	...
	if(game_mode)
        ...
        if(!edit_cfg)
            ...
            if(edit_cfg==1)
				...
				if(Draw_button2(m, g, pollaenvinagre/*"  A�adir PNG   "*/, (mode_disc & 3) ? -1 : 129)) select_game_bar=n; n++;g+=56;
